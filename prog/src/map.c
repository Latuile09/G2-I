...
{
	switch (type & 0x01) { // sub-types are encoded with the 1 less significant bit
	case DOOR_CLOSE:
...
{
	FILE* map_file=NULL;
	map_file=fopen("data/map_1","r");
	int dimensions[2];
	if (map_file!=NULL)
		{
		fscanf(map_file,"%d:%d",&dimensions[0],&dimensions[1]);
		}
	else
		{
		printf("no file for map");
		}
	struct map* map = map_new(dimensions[0],dimensions[1]);
	int nb_cell=dimensions[0]*dimensions[1];
	int themap[nb_cell];
	int i=0;
	for(i=0; i<nb_cell;i=i+1)
		{
		  fscanf(map_file, "%d ", &themap[i]);
		}
	fclose(map_file);
	int k=0;
	for(int j=0; j<dimensions[1];j++)
		...
		for(int i=0; i<dimensions[0];i++)
			{
			map_set_cell_type(map,i,j,themap[k]);
			printf("%d,     %d\n",themap[k],k);
			k++;
			}
		}
...
{
	FILE* map_file=NULL;
	char* name_level=malloc(sizeof(name_level));
	sprintf(name_level,"data/map_%d",level);
	//printf("%s",name_level);
	map_file=fopen(name_level,"r");
	int dimensions[2];
	if (map_file!=NULL)
		{
		fscanf(map_file,"%d:%d",&dimensions[0],&dimensions[1]);
